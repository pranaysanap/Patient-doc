...
const HospitalLocator: React.FC = () => {
    ...
    const getUserLocation = useCallback(() => {
        ...
        if (!navigator.geolocation) {
            // Browser doesn't support geolocation - use Terna College
            const location = ternaCollegeLocation;
            setUserLocation(location);
            setMapCenter(location);
            setLocationName('Terna Engineering College, Nerul (Demo Location)');
            if (mapRef.current) {
                mapRef.current.panTo(location);
                mapRef.current.setZoom(14);
            }
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
                // FAILED - User denied or error - fallback to Terna College for demo
    ...
    const getMarkerIcon = (type: 'government' | 'private' | 'unknown') => {
        const color = type === 'government' ? '#ec4899' : '#ef4444';
        return {
            url: 'data:image/svg+xml,' + encodeURIComponent(`
                <svg width="48" height="56" viewBox="0 0 48 56" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path d="M24 4C13.507 4 6 11.954 6 22c0 14 18 28 18 28s18-14 18-28C42 11.954 34.493 4 24 4z" fill="${type === 'government' ? '#f472b6' : '#f87171'}"/>
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-xl font-bold bg-gradient-to-r from-pink-500 to-red-600 bg-clip-text text-transparent">Hospital Locator</h1>
