...
import { AIAssistantService, Message, getAIAssistant, DR_VAIDYA_SYSTEM_PROMPT as SYSTEM_PROMPT } from '@/lib/ai-assistant';
...
const VaidyaContext = React.createContext<VaidyaContextType | null>(null);

// Hook to use the assistant
export function useVaidya() {
    const context = React.useContext(VaidyaContext);
    if (!context) {
...
export function VaidyaButton() {
    const { openAssistant } = useVaidya();
...
function VaidyaDialog() {
    const { closeAssistant, messages, sendMessage, isTyping, clearMessages } = useVaidya();
    ...
    const handleDownloadChat = () => {
        try {
            ...
            const doc = new jsPDF();
            doc.setFontSize(18);
            doc.text("Dr. Vaidya Chat History", 20, 20);
            ...
            messages.slice(1).forEach((msg) => {
                ...
                doc.text(`${msg.role === "user" ? "You" : "Dr. Vaidya"} - ${msg.timestamp.toLocaleTimeString()}`, 20, yPosition);
                yPosition += 5;
            ...
            >
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h2 className="text-xl font-semibold">Dr. Vaidya - AI Health Assistant</h2>
                ...
                <ScrollArea className="flex-1 p-4">
                    <div className="space-y-4">
                        {messages.slice(1).map((message) => (
                            ...
                            >
                                <div
                                    className={`relative max-w-[80%] px-4 py-3 rounded-lg ${message.role === "assistant"
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <button
                                        ...
                                        className={`rounded-full p-2 ${textToSpeechEnabled ? "text-blue-500" : "text-muted-foreground"
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <button
                                        ...
                                        className={`rounded-full p-2 ${isListening ? "text-red-500" : "text-muted-foreground"
                                            } hover:bg-accent transition-colors`}
                ...
                <div className="px-4 py-2 border-t flex items-center justify-between text-xs text-muted-foreground">
                    <p>Dr. Vaidya provides general information and is not a substitute for professional medical advice.</p>
...
export function VaidyaAssistant() {
    return (
        <VaidyaProvider>
            <VaidyaButton />
        </VaidyaProvider>
